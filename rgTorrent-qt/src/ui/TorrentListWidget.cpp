...
void TorrentListWidget::rebuildTable() {
    ...
    const QString statusError = QCoreApplication::translate("TorrentBackend", "Error");
...
QString TorrentListWidget::formatSize(const quint64 bytes) {
    if (bytes == 0) return QStringLiteral("—");
    const double kb = 1024, mb = kb * 1024;

    if (const double gb = mb * 1024; bytes >= gb) return tr("%1 GB").arg(bytes / gb, 0, 'f', 2);
    if (bytes >= mb) return tr("%1 MB").arg(bytes / mb, 0, 'f', 1);
    if (bytes >= kb) return tr("%1 KB").arg(bytes / kb, 0, 'f', 0);
    return tr("%1 B").arg(bytes);
}

void TorrentListWidget::retranslateUi() const {
    m_table->setHorizontalHeaderLabels({
        tr("Name"), tr("Size"), tr("Status"),
        tr("Seeds"), tr("Peers"), tr("Progress"), tr("Seeding")
    });
